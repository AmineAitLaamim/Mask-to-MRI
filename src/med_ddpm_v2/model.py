...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, condition_tensors=None, noise=None):
        ...
        Original: concatenate mask with noisy image before feeding to U-Net.
        Loss: L1 or L2 between predicted noise and actual noise.
        """
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        if self.loss_type == "l1":
            ...
            raise NotImplementedError(f"loss_type={self.loss_type}")
...
class ConditionalDDPM(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.denoise_fn = UNetModel(
            ...
            in_channels=config["in_channels"],   # 4 = 3 MRI + 1 mask
            model_channels=config["num_channels"],
            out_channels=config["out_channels"],  # 3 = predicted noise ε
        ...
        self.diffusion = GaussianDiffusion(
            ...
            image_size=config["image_size"],
            channels=config["out_channels"],  # 3
